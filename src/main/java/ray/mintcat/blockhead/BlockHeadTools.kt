...
object BlockHeadTools {
    fun openTradeUI(player: Player, recipeList: List<TradingRecipe>) {
        val TradeUI: String? = BlockHead.settings.getString("GUI.TradeUI", "....")
                .replace("&", "§")
                .replace("{Items}", recipeList.get(0).getResultItem().getItemMeta().getDisplayName())
                .replace("{Shopkeeper}", BlockHeadObject.titleNameMap[player.getUniqueId()]!!)
        val recipes: MutableList<TradingRecipe> = ArrayList()
        for (i in 1 until recipeList.size) {
            recipes.add(recipeList.get(i))
        }
        NMSManager.getProvider().openTradeWindow(TradeUI, recipes, player)
    ...
    fun runCommands(player: Player, item: ItemStack) {
        if (item.itemMeta.lore.size != 0) {
            val lores = item.itemMeta.lore.toTypedArray()
    ...
    fun runStart(player: Player, item: ItemStack) {
        when (item.type) {
            ...
            Material.COMMAND_MINECART -> runCommands(player, item)
    ...
    fun baseImport(player: Player,command: String){
        ...
        Signs.fakeSign(player, Consumer { lines: Array<String?>? ->
            ...
            player.openInventory(BlockHeadObject.inventoryMap[player.uniqueId])
        })
    }
